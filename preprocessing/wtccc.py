...
Returns both the genotype matrix and the individual IDs (FID, IID) in matching row
order, read from the QC'd `.fam` (which matches the `.raw`/`.npy` order). Downstream
code (vae/train_wtccc.py) embeds these IDs in the latent CSV so heritability
construction joins individuals by ID rather than by row position.

Example usage:
    from preprocessing.wtccc import load_wtccc_data
    # Unpruned (default)
    data, ids = load_wtccc_data(raw_prefix="/work/long_lab/for_Ariel/BD", disease_name="BD")
    # Pruned
    data, ids = load_wtccc_data(raw_prefix="/work/long_lab/for_Ariel/BD", disease_name="BD", prune=True)
"""
...
def _read_ids(qc_prefix):
    """Read FID/IID from the QC'd .fam (matches the .raw / .npy row order)."""
    return pd.read_csv(f"{qc_prefix}.fam", sep=r"\s+", header=None,
...
def load_wtccc_data(raw_prefix, disease_name, prune=False):
    ...
    (numpy.ndarray, pandas.DataFrame)
        The imputed genotype matrix and a FID/IID table in matching row order.
    """
    ...
    if os.path.exists(final_npy):
        ...
        return np.load(final_npy), _read_ids(qc_prefix)
    ...
    return arr, _read_ids(qc_prefix)
